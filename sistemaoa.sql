...
CREATE TABLE `comentario` (
  ...
  `idComentarioSup` int(11),
  PRIMARY KEY (`idComentario`),
  KEY `idOA` (`idOA`),
  KEY `idUsuario` (`idUsuario`),
  KEY `idComentarioSup` (`idComentarioSup`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE PROCEDURE `insertarComentario` (
	IN detalleN text,
    IN idOAN int(11),
    IN idUsuarioN int(11))
BEGIN

	INSERT INTO comentario (detalleComent, idOA, idUsuario)
            VALUES (detalleN, idOAN, idUsuarioN);
...
BEGIN
	...
	INSERT INTO comentario (detalleComent, idOA, idUsuario, tipoUsuario)
            VALUES (detalleN, idOAN, idUsuarioN, tipoUsuarioN);
...
-- select comentarioXusuario

DROP procedure IF EXISTS `seleccionarComentarioDeUnUsuario`;

DELIMITER $$
CREATE PROCEDURE `seleccionarComentarioDeUnUsuario` (
	IN tipoUsuario varchar(1),
    IN idOAN int(11) 
)
BEGIN

	IF STRCMP(tipoUsuario, 'p') THEN SELECT detalleComent, nombresProf, apellidosProf FROM comentario c JOIN profesor p	ON p.idProfesor = c.idUsuario WHERE idOA = idOAN;
	ELSEIF STRCMP(tipoUsuario, 'e') THEN SELECT detalleComent, nombresProf, apellidosProf FROM comentario c JOIN estudiante e ON e.idEstudiante = c.idUsuario WHERE idOA = idOAN;
	END IF;
